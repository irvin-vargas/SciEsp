...
public class DescriptiveAnalyzer implements ICohAnalyzer{
	...
	public CohStats numberOfWordsInSentences(CohText text) {
		...
		//System.out.println("valor = "+valor.size());

		for(int j=0;j<indice.size();j++)
		{
			cad = evaluar(valor.get(j));
			int ultimo = valor.size();
			int penultimo = valor.size()-1;
			if (indice.get(j)==1)
					cad += ",Primera\n";
			else if(indice.get(j)==2)
					cad += ",Segunda\n";
			else if (indice.get(j)==penultimo)
					cad += ",Penúltima\n";
			else if(indice.get(j)==ultimo)
					cad += ",Última\n";
			else
					cad += ",Mediana\n";
			try
